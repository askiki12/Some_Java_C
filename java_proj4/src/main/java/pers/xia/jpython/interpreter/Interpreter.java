...
public class Interpreter {
    ...
    private Statement parseAstNode(stmtType node){
        ...
        if(node instanceof Assign){
            ...
            int i=0;
            while(i<((Assign) node).targets.size()) {
                String the_variableName;
                exprType target = ((Assign) node).targets.get(i);
                the_variableName = parser.getNameVal((Name) target);
                list.add(the_variableName);
                i++;
            }
            String[] variableName = list.toArray(new String[0]);
//            String variableName;
